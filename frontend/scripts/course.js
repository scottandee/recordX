...
function doUpdate (course) {
  ...
  $.ajax({
    type: 'GET',
    url: 'http://127.0.0.1:5000/api/v1/departments/' + course.department_id,
    success: (dept) => {
  ...
  updateFormEl.on('submit', function (event) {
    ...
    $.ajax({
      type: 'PUT',
      url: 'http://127.0.0.1:5000/api/v1/courses/' + course.id,
      dataType: 'json',
...
$(document).ready(() => {
  $.ajax({
    type: 'POST',
    url: 'http://127.0.0.1:5000/api/v1/courses_search',
    datatype: 'json',
    contentType: 'application/json',
    data: JSON.stringify({}),
    success: (courses) => {
